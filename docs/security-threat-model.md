...
|---|---|---|---|---|
| 1 | Provider API key extraction from APK | Android | Keys live **only** on backend. A production gateway with short-lived install credentials is still required. | Partial |
...
| 8 | Debug logging in release | Android + Backend | No content logging is allowed. CI should add an automated content-log check before public release. | Partial |
...
| 11 | Man-in-the-middle on backend requests | Android + Backend | Production must use HTTPS. Cleartext is allowlisted only for loopback development addresses. | Partial |
| 12 | Replay attacks on backend endpoints | Backend | Not yet mitigated; short-lived credentials and gateway replay protection are required before public deployment. | Open |
| 13 | Abuse of public backend endpoints | Backend | Bounded in-memory source/install rate key plus request-size and duration caps. Distributed gateway limiting is required for scale. | Partial |
...
| 21 | Malformed WAV metadata / parser abuse | Backend | Validate PCM format, channel/rate bounds, chunk bounds, upload size, and duration before provider calls. | **Implemented ✅** |
| 22 | Unsafe production configuration | Backend | Production startup requires provider/client secrets and rejects wildcard CORS. | **Implemented ✅** |
| 23 | Rate-limit memory exhaustion | Backend | Hash identifiers and enforce a fixed maximum number of active buckets. | **Implemented ✅** |
